...
#ifdef USE_UNPACK_UINT_2X32
    uvec2 parts = unpackUint2x32(prod);
    uint diff = parts.y - parts.x;
#else
...
uint[16] finalize_checksum(uint[16] state, uint y, uint x) {
    uint y_top5bits = y >> 27;
    uint y_bottom5bits = y & 0x1F;
...
void main() {
    ...
    uint state[16] = finalize_checksum(state_in, y, x);
